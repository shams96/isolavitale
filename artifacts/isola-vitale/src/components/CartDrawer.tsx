...
export default function CartDrawer() {
  ...
  const mappedUpsell = {
    ...
    cartId: upsellItem.id,
    name: upsellItem.name,
    price: upsellItem.fullPrice,
    imageSrc: upsellItem.imageSrc || (upsellItem as any).image,
    variant: 'Standard'
  };
            ...
            cart.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className={styles.item}>
                <div className={styles.imageContainer}>
                  <img src={item.imageSrc || item.image} alt={item.name} style={{ objectFit: 'cover', width: '100%', height: '100%' }} />
                </div>
            ...
            {upsells.map(upsell => (
              <div key={upsell.id} className={styles.upsellItem}>
                <div className={styles.upsellImage}>
                  <img src={upsell.imageSrc || (upsell as any).image} alt={upsell.name} style={{ objectFit: 'cover', width: '100%', height: '100%' }} />
                </div>
